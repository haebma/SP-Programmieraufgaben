...
/*I----> +--------------------------------------------------------------------+
         | Fehlerbehandlung für `plistGet` fehlt (-1)                         |
         +-------------------------------------------------------------------*/
...
static int printZombies(const ProcInfo *info, int event){
    ...
    if(WIFSIGNALED(event) || WIFEXITED(event)){
        ...
        handleSig(SIGCHLD, SIG_UNBLOCK);
/*I----> +--------------------------------------------------------------------+
         | SIGCHLD wird hier ggf. früher wieder freigegeben, als von der      |
         | aufrufenden Funktion vorhergesehen. Deswegen sollte die alte Maske |
         | beim `sigprocmask`-Aufruf ausgelesen und hinterher                 |
         | wiederhergestellt werden. (-0.5)                                   |
         +-------------------------------------------------------------------*/
    }
...
static int nukeEveryone(const ProcInfo *infos){
    if(kill(infos->pid, SIGTERM) == -1) {
        ...
        return -1;
/*I----> +--------------------------------------------------------------------+
         | Da diese Funktion in einer `plistIterate`-Schleife aufgerufen wird |
         | führt ein Rückgabewert von -1 dazu, dass nicht alle Prozesse       |
         | bearbeitet werden (siehe `plistIterate` Doku). (-0.5)              |
         +-------------------------------------------------------------------*/
    }
    return 0;
}
...
/*I      +---------A---------------------------------------------------------+
         | Diese Bedingung wird nie wahr werden, da eine Variable vom Typ     |
         | `long` nie größer als LONG_MAX sein wird.                          |
         +-------------------------------------------------------------------*/
...
/*I----> +--------------------------------------------------------------------+
         | 0 ist ein gültiger Wert. Er bedeutet, dass alle Prozesse in der    |
         | Gruppe beendet werden (siehe kill(1), kill(3p))                    |
         +-------------------------------------------------------------------*/
            fprintf(stderr, "nuke: Pid argument is out of range\n");
            return;
        }
...
/*I----> +--------------------------------------------------------------------+
         | Für lesenden Zugriff ist es nicht nötig, die Berechtigungen        |
         | anzugeben.                                                         |
         +-------------------------------------------------------------------*/
        ...
        if(close(inFile) == -1) die("close");
        if(close(outFile) == -1) die("close");
/*I      +-A-----------------------------------------------------------------+
         | `inFile`/`outFile` sind hier möglicherweise nicht initialisiert,   |
         | was zu einer verfälschten Fehlerausgabe führt. (-1)                |
         +-------------------------------------------------------------------*/
...
/*I----> +--------------------------------------------------------------------+
         | Es fehlt das blockieren und wieder freigeben von `SIGCHLD`         |
         | sigprocmask(3p). (-1)                                              |
         +-------------------------------------------------------------------*/
            ...
            while(foreGroundProc){ // solange schlafen, bis der Vordergrundprozess nicht mehr läuft
                ...
                errno = 0;
/*I----> +--------------------------------------------------------------------+
         | Die errno muss _in_ der Signalbehandlung gesichert werden (was sie |
         | bei euch auch korrekter Weise wird), nicht _davor_.                |
         +-------------------------------------------------------------------*/
...
/*I----> +--------------------------------------------------------------------+
         | sigprocmask(3p) Verzögert Signale nur bis sie wieder freigegeben   |
         | werden. Das kann hier dazu führen, dass SIGINT-Signale deutlich    |
         | später zugestellt werden, als vom Nutzer erwartet. Richtig wäre    |
         | hier ein Aufruf an sigaction(3p) mit `SIG_IGN` als Parameter.      |
         | (-0.5)                                                             |
         +-------------------------------------------------------------------*/
    handleSigchld();
...
/*P----> +--------------------------------------------------------------------+
         | Punktabzug in dieser Datei: 4.5 Punkte                             |
         +-------------------------------------------------------------------*/
