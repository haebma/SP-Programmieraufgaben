...
int queue_put(char *cmd, char *out, int flags) { // no copying just storing
    ...
    if (entry == NULL){ // error in malloc -> errno is set
        V(sem1);
        return -1;
    }
//printf("Vor strdup\n");
    ...
    if (out != NULL){
	entry->out = strdup(out);
    }
    if (errno) { // Fehler in strdup
	return -1;
    }
//printf("Nach strdup\n");
    entry->flags = flags;
...
//    printf("Queue_put-Fkt: Folgendes wurde eingefuegt:\n Command: %s\n Output: %s\n Flags: %d\n", entry->cmd, entry->out, entry->flags);
    return 0; // success
}
 ...
 * @return @c 0 on success, any other value on error (sets @c errno on error)
 */
int queue_get(char **cmd, char **out, int *flags) { // removes first element; cmd and out must be freed be caller
    ...
    errno = 0;
    if(entry->cmd != NULL){
        *cmd = strdup(entry->cmd);
    }
    if(entry->out != NULL){
        *out = strdup(entry->out);
    }
    if(errno){ // Fehler in strdup
        return -1;
    }
 ...
 if (entry->cmd != NULL) free(entry->cmd);
 if (entry->out != NULL) free(entry->out);
 free(entry);
	
//printf("Queue_get-Fkt: Pointer beschrieben mit:\n Command: %s\n Out: %s\n Flags: %d\n", *cmd, *out, *flags);
    V(sem1);
    return 0; // success
}
